...
def main(args):
    ...
    for dataset_name, cols_of_interest in DATASETS.items():
        ...
        filtered_df_dir = args.input_dir
        logging.info(f"filtered df dir: {filtered_df_dir}")
        if not os.path.exists(filtered_df_dir):
        ...
        for split in ["all"]:
            ...
            filtered_file_path = os.path.join(
                filtered_df_dir, f"{cleaned_dataset_name}_{split}_filtered.parquet"
            )
            ...
            original_output_path = os.path.join(args.output_dir, "original.parquet")
            brand_to_generic_output_path = os.path.join(
                args.output_dir, "brand_to_generic.parquet"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default="data/coral/pre_edit",
        help="Directory containing the filtered Parquet files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="data/coral/post_edit",
